...
def get_data_path(subpath=''):
    ...
    """
    return Path(get_package_share_directory('global_planner')).parents[3] / 'src/race_stack/planner/global_planner' / subpath
...
def extract_track_bounds(
        ...
        show_plots: bool) -> Tuple[np.ndarray, np.ndarray]: # was tuple
    """
    ...
    for label in np.unique(labels):
        ...
        # If more than 2 closed contours detected, select the 2 longest ones
        if len(closed_contours) < 2:
            raise IOError("Less than two track bounds detected! Check input")
        elif len(closed_contours) > 2:
            # Sort by contour length and take the two longest
            closed_contours = sorted(closed_contours, key=len, reverse=True)[:2]
