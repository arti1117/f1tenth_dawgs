...
from .readwrite_global_waypoints import write_global_waypoints, write_waypoints_to_csv
...
class GlobalPlannerLogic:
    ...
    def compute_global_trajectory(self, cent_length: float) -> bool:
        ...
        """
        self.loginfo("=" * 60)
        self.loginfo("STARTING TRAJECTORY COMPUTATION")
        self.loginfo("=" * 60)

        # open up png map from file
        self.loginfo("Step 1/10: Loading map PNG file...")
        img_path = os.path.join(self.map_dir, self.map_name + '.png')
        filtered_map = cv2.flip(cv2.imread(img_path, 0), 0)
        self.loginfo(f"  ✓ Loaded map from: {img_path}")
        self.loginfo(f"  ✓ Map size: {filtered_map.shape}")

        # skeletonize
        self.loginfo("Step 2/10: Skeletonizing map (this may take 10-30 seconds)...")
        skeleton = skeletonize(filtered_map, method='lee')
        self.loginfo(f"  ✓ Skeleton created: {skeleton.shape}")

        if self.show_plots:
            self.loginfo("Step 3/10: Displaying skeleton plot (close window to continue)...")
            ...
            plt.show()
            self.loginfo("  ✓ Plot closed")
        else:
            self.loginfo("Step 3/10: Skipping plot display (show_plots=False)")

        ################################################################################################################
        # Extract centerline from filtered occupancy grid map
        ################################################################################################################
        self.loginfo("Step 4/10: Extracting centerline from skeleton...")
        try:
            ...
            self.loginfo(f"  ✓ Centerline extracted: {centerline.shape[0]} points")
        except IOError:
            if self.map_editor_mode:
        ...
        self.loginfo("Step 5/10: Smoothing and converting centerline...")
        centerline_smooth = smooth_centerline(centerline)
        self.loginfo(f"  ✓ Centerline smoothed: {centerline_smooth.shape[0]} points")
        ...
        self.loginfo(f"  ✓ Converted to meters (resolution: {self.map_resolution}m)")
        ...
        self.loginfo(f"  ✓ Interpolated to 0.1m steps: {centerline_meter_int.shape[0]} points")
        ...
        self.loginfo("Step 6/10: Extracting track bounds...")
        if self.watershed:
            try:
                self.loginfo("  Attempting watershed algorithm...")
            ...
            bound_l_water = None
            dist_transform = cv2.distanceTransform(filtered_map, cv2.DIST_L2, 5)
            self.loginfo('  ✓ Distance transform computed')

        ################################################################################################################
        # Compute global trajectory with mincurv_iqp optimization
        ################################################################################################################
        self.loginfo("=" * 60)
        self.loginfo("Step 7/10: IQP TRAJECTORY OPTIMIZATION")
        self.loginfo("=" * 60)
        ...
        self.loginfo("  Adding distance to centerline bounds...")
        ...
        self.loginfo(f"  ✓ Centerline with bounds prepared (safety_width={self.safety_width}m)")
        ...
        # Ensure el_lengths is explicitly 1-D
        el_lengths = (0.1 * np.ones(len(centerline_coords) - 1)).ravel()
        ...
        self.loginfo('  Running IQP optimizer (this may take 1-3 minutes)...')
        try:
            ...
            self.loginfo(f"  ✓ IQP optimization complete!")
            self.loginfo(f"    - Estimated lap time: {est_t_iqp:.2f}s")
            self.loginfo(f"    - Max speed: {np.amax(global_trajectory_iqp[:, 5]):.2f} m/s")
            self.loginfo(f"    - Trajectory points: {global_trajectory_iqp.shape[0]}")
        except RuntimeError as e:
            self.logwarn(f"  ✗ Error during IQP optimization: {e}")
        ...
        ################################################################################################################
        self.loginfo("=" * 60)
        self.loginfo("Step 8/10: SHORTEST PATH OPTIMIZATION (for overtaking)")
        self.loginfo("=" * 60)

        self.loginfo('  Running IQP for overtaking trajectory...')
        ...
        self.loginfo(f"  ✓ Overtaking IQP complete (safety_width={self.safety_width_sp}m)")
        ...
        self.loginfo('  Running shortest path optimizer (this may take 1-3 minutes)...')
        global_trajectory_sp, bound_r_sp, bound_l_sp, est_t_sp = trajectory_optimizer(
            input_path=self.input_path, track_name=sp_centerline_path, curv_opt_type='shortest_path', safety_width=self.safety_width_sp, plot=(
                self.show_plots and not self.map_editor_mode))
        self.loginfo(f"  ✓ Shortest path optimization complete!")
        self.loginfo(f"    - Estimated lap time: {est_t_sp:.2f}s")
        self.loginfo(f"    - Max speed: {np.amax(global_trajectory_sp[:, 5]):.2f} m/s")
        self.loginfo(f"    - Trajectory points: {global_trajectory_sp.shape[0]}")
        ...
        self.loginfo("  ✓ JSON file saved successfully")

        # Save waypoints to CSV for path_planner usage
        self.loginfo("")
        self.loginfo("=" * 60)
        self.loginfo("Step 10/10: Exporting waypoints to CSV for path_planner")
        self.loginfo("=" * 60)
        ...
        self.loginfo("")
        self.loginfo("=" * 60)
        self.loginfo("TRAJECTORY GENERATION COMPLETE!")
        self.loginfo("=" * 60)
        self.loginfo(f"Summary:")
        self.loginfo(f"  Map: {self.map_name}")
        self.loginfo(f"  IQP lap time: {est_t_iqp:.2f}s")
