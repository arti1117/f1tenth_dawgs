...
def trajectory_optimizer(input_path: str,
    ...
    # dawgs need to change this
    file_paths = {"veh_params_file": "racecar_dawgs.ini"}
    ...
    config_path = os.path.join(file_paths["inputs"], file_paths["veh_params_file"])
    if not parser.read(config_path):
        raise ValueError(f'Specified config file does not exist or is empty! Path:{config_path}')
    ...
    # PREPARE REFTRACK -------------------------------------------------------------------------------------------------
    # ------------------------------------------------------------------------------------------------------------------
    # Debug: Check reftrack_imp shape and contents
    print(f"DEBUG trajectory_optimizer: reftrack_imp.shape = {reftrack_imp.shape}")
    print(f"DEBUG trajectory_optimizer: reftrack_imp.dtype = {reftrack_imp.dtype}")
    print(f"DEBUG trajectory_optimizer: First 3 rows of reftrack_imp:\n{reftrack_imp[:3]}")

    # Ensure reftrack_imp is a proper 2D numpy array
    ...
    # Note: a_interp is the LES coefficient matrix from calc_splines and is SUPPOSED to be 2D
    # Do not flatten it - it's passed as matrix A to optimization functions
    ...
    if curv_opt_type == 'mincurv':
        ...
        print(f"DEBUG: reftrack_interp.shape = {reftrack_interp.shape}")
        print(f"DEBUG: normvec_normalized_interp.shape = {normvec_normalized_interp.shape}")
        print(f"DEBUG: a_interp type = {type(a_interp)}")
        if hasattr(a_interp, 'shape'):
            print(f"DEBUG: a_interp.shape = {a_interp.shape}")
            print(f"DEBUG: a_interp.ndim = {a_interp.ndim}")
            # If A is 2D, we might need to handle it differently
            if a_interp.ndim == 2:
                print(f"WARNING: a_interp is 2D with shape {a_interp.shape}")
                print(f"DEBUG: First few rows of a_interp:\n{a_interp[:3]}")

        print(f"DEBUG: Creating arrays with shape[0] = {reftrack_interp.shape[0]}")

        # Calculate spline lengths from reftrack
        # For closed track: el_lengths should have same length as path points
        spline_len_array = np.sqrt(np.sum(np.diff(reftrack_interp[:, :2], axis=0)**2, axis=1))
        # Add closing segment (last point to first point)
        first_seg_len = np.sqrt(np.sum((reftrack_interp[0, :2] - reftrack_interp[-1, :2])**2))
        el_lengths = np.append(spline_len_array, first_seg_len)

        print(f"DEBUG: reftrack_interp.shape[0] = {reftrack_interp.shape[0]}")
        print(f"DEBUG: el_lengths.shape[0] = {el_lengths.shape[0]}")
        print(f"DEBUG: These should be equal for closed track!")

        # Calculate heading and curvature numerically using TPH
        psi_array, kappa_array = tph.calc_head_curv_num.calc_head_curv_num(
            path=reftrack_interp[:, :2],
            el_lengths=el_lengths,
            is_closed=True
        )

        # Calculate derivative of curvature
        dkappa_array = np.diff(kappa_array) / el_lengths[:-1]
        # Add last element by wrapping
        dkappa_array = np.append(dkappa_array, (kappa_array[0] - kappa_array[-1]) / el_lengths[-1])

        print(f"DEBUG: el_lengths.shape = {el_lengths.shape}")
        print(f"DEBUG: psi_array.shape = {psi_array.shape}")
        print(f"DEBUG: kappa_array.shape = {kappa_array.shape}")
        print(f"DEBUG: dkappa_array.shape = {dkappa_array.shape}")
        print(f"DEBUG: el_lengths range: [{np.min(el_lengths):.4f}, {np.max(el_lengths):.4f}]")
        print(f"DEBUG: psi_array range: [{np.min(psi_array):.4f}, {np.max(psi_array):.4f}]")
        print(f"DEBUG: kappa_array range: [{np.min(kappa_array):.4f}, {np.max(kappa_array):.4f}]")
        ...
        try:
            ...
            print(f"ERROR in iqp_handler call: {e}")
    ...
    if curv_opt_type == 'mintime' and not mintime_opts["recalc_vel_profile_by_tph"]:
        ...
        # ensure xp and fp are 1-D arrays (defensive)
        xp = np.asarray(s_splines[:-1]).ravel()
        fp = np.asarray(v_opt).ravel()

        # optional debug print to help future debugging (주석 처리하거나 로그 레벨에 따라 출력)
        print("DEBUG: shapes for interp -> s_points_opt_interp.shape:", np.shape(s_points_opt_interp),
            "xp.shape:", xp.shape, "fp.shape:", fp.shape)

        vx_profile_opt = np.interp(s_points_opt_interp, xp, fp)
